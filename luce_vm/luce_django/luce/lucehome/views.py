...
class UploadDataView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if not LuceRegistry.objects.filter(pk=1).exists():
            response = custom_exeptions.custom_message("luce registry was not deployed")
            return Response(response["body"], response["status"]) 
        ...
        if not restriction_serializer.is_valid():
            response = custom_exeptions.validation_exeption(restriction_serializer)
            return Response(response["body"], response["status"]) 
        if not serializer.is_valid():
            response = custom_exeptions.validation_exeption(serializer)
            return Response(response["body"], response["status"])
        ...
        if type(tx_receipt2) is list:
                datacontract.delete()
                response = custom_exeptions.blockchain_exception(tx_receipt2, tx_receipts)
                return Response(response["body"], response["status"]) 
        ...
        if type(tx_receipt3) is list:
                datacontract.delete()
                response = custom_exeptions.blockchain_exception(tx_receipt3, tx_receipts)
                return Response(response["body"], response["status"]) 
        ...
        if type(tx_receipt4) is list:
                datacontract.delete()
                response = custom_exeptions.blockchain_exception(tx_receipt4, tx_receipts)
                return Response(response["body"], response["status"]) 
    ...
    def request_access(self, c_address, request, access_time, estimate, purpose_code):
        ...
        if is_registered==0:
            ...
            if type(receipt) is list:
                    ...
                    response = custom_exeptions.blockchain_exception(receipt, tx_receipts)
                    return Response(response["body"], response["status"])
            tx_receipts.append(receipt)

        receipt2 = datacontract.consent_contract.give_general_research_purpose(request.user, estimate)
        if type(receipt2) is list:
                datacontract.consent_contract.research_purpose.delete()
                response = custom_exeptions.blockchain_exception(receipt2, tx_receipts)
                return Response(response["body"], response["status"])
        tx_receipts.append(receipt2)

        receipt3 = datacontract.consent_contract.give_HMB_research_purpose(request.user, estimate)
        if type(receipt3) is list:
                datacontract.consent_contract.research_purpose.delete()
                response = custom_exeptions.blockchain_exception(receipt3, tx_receipts)
                return Response(response["body"], response["status"])
        tx_receipts.append(receipt3)

        receipt4 = datacontract.consent_contract.give_clinical_research_purpose(request.user, estimate)
        if type(receipt4) is list:
                datacontract.consent_contract.research_purpose.delete()
                response = custom_exeptions.blockchain_exception(receipt4, tx_receipts)
                return Response(response["body"], response["status"])
        tx_receipts.append(receipt4)

        receipt5 = datacontract.add_data_requester(access_time,purpose_code, request.user, estimate)
        if type(receipt5) is list:
                datacontract.consent_contract.research_purpose.delete()
                response = custom_exeptions.blockchain_exception(receipt5, tx_receipts)
                return Response(response["body"], response["status"])
...
class SearchContract(APIView):
    ...
    def post(self, request, format = None):
        ...
        final_result = []
        contracts = DataContract.objects.filter(description__contains = searchparam)
...
class LuceRegistryView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if user.ethereum_public_key is None or user.ethereum_private_key is None:
            response = custom_exeptions.custom_message("user must connect a wallet first")
            return Response(response["body"], response["status"]) 
